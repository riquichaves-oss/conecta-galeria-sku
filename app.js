(() => {
  ...
  // Helpers
  const isCodeValid = (c) => /\d+\.\d+\.\d+\.\d+/.test(String(c || "").trim());

  function App() {
    const [rows, setRows] = useState([]);        // [{codigo, descricao, estoque?, custo?}]
    const [mapping, setMapping] = useState({});  // {codigo: [dataURL,...]}
    ...
    // load initial items from items.json
    useEffect(() => {
      (async () => {
        try {
          ...
          if (Array.isArray(data?.items)) {
            const clean = data.items
              .filter((r) => isCodeValid(r.codigo))
              .map((r) => ({
                codigo: String(r.codigo).trim(),
                descricao: String(r.descricao || "").trim(),
                // mantém campos mesmo que não exibidos
                estoque: Number(r.estoque || 0),
                custo:
                  r.custo === undefined || r.custo === null || r.custo === ""
                    ? undefined
                    : Number(r.custo),
              }));
            setRows(clean);
            setStatus(`Itens carregados do estoque: ${clean.length}`);
          }
        } catch (e) {
          console.warn("Falha ao carregar items.json", e);
        }
      })();
    }, []);
    ...
    const handleUploadPlan = async (file) => {
      ...
      const mapRow = (row) => {
        ...
        const codigo = String(
          get("Código", "Codigo", "CÓDIGO", "codigo", "código")
        ).trim();
        if (!isCodeValid(codigo)) return null;
        return {
          codigo,
          descricao: String(
            get(
          ...
          custo:
            Number(String(get("Custo", "Custo Unit", "Preco")).replace(",", ".")) ||
            undefined,
        };
      };
    ...
    const importJSON = async (file) => {
      ...
      try {
        ...
        if (Array.isArray(obj?.items)) setRows(obj.items);
    ...
    const exportZIP = async () => {
      ...
      for (const r of rows) {
        ...
        fotos.forEach((dataUrl, i) => {
          ...
          const ext = (meta?.match(/data:image\/(.*?);/)?.[1] || "png").replace(
            "jpeg",
            "jpg"
          );
    ...
    // ---- Adicionar / Excluir SKUs ----
    const handleAddSKU = () => {
      const codigo = fCodigo.trim();
      const descricao = fProduto.trim();
      const custo =
        fCusto === "" ? undefined : Number(String(fCusto).replace(",", "."));

      if (!isCodeValid(codigo)) {
        setStatus("Código inválido. Use o formato 000.00.00.0000.");
        return;
      }
      if (!descricao) {
      ...
      if (rows.some((r) => r.codigo === codigo)) {
        setStatus("Já existe um item com esse código.");
        return;
      }

      setRows((old) => [
        ...old,
        { codigo, descricao, estoque: 0, custo },
      ]);
      setFCodigo("");
      setFProduto("");
      setFCusto("");
      setStatus("SKU adicionado. Use Exportar JSON para salvar a base.");
    };

    const handleDeleteSKU = (codigo) => {
      if (!confirm(`Excluir SKU ${codigo}?`)) return;
      setRows((old) => old.filter((r) => r.codigo !== codigo));
      const copy = { ...mapping };
      delete copy[codigo];
      setMapping(copy);
      setStatus("SKU excluído.");
    };
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "Itens com foto: ",
            React.createElement("b", null, totalComFoto)
          )
        )
      ),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "O site já carrega os SKUs atuais. Se quiser atualizar a base via planilha, envie um Excel (",
            React.createElement("b", null, "Código"),
            ", ",
            React.createElement("b", null, "Descrição"),
            ", ",
            React.createElement("b", null, "Qtde"),
            ", ",
            React.createElement("b", null, "Custo"),
            ")."
          ),
          React.createElement("input", {
            type: "file",
            accept: ".xlsx,.xls",
            onChange: (e) => e.target.files?.[0] && handleUploadPlan(e.target.files[0]),
          }),
          status ? React.createElement("p", { className: "text-xs text-gray-500 mt-2" }, status) : null
        ),

        // 2) Filtros + Upload em lote
        React.createElement(
          "section",
          { className: "bg-white rounded-2xl shadow p-4 grid gap-3 md:grid-cols-3" },
          React.createElement(
            "div",
            null,
          ...
          React.createElement(
            ...
            " Mostrar somente itens ",
            React.createElement("b", null, "sem foto")
          ),
          React.createElement(
            "div",
            null,
            ...
            React.createElement("input", {
              ...
              accept: "image/*",
              multiple: true,
              onChange: (e) => handleBulkMatch(e.target.files),
            }),
            React.createElement(
              "p",
              { className: "text-xs text-gray-500 mt-1" },
              "Arquivos são vinculados se o nome contiver o código (ex.: 006.01.00.1004_friso.jpg)."
            )
          )
        ),
        ...
        React.createElement(
          "section",
          { className: "bg-white rounded-2xl shadow p-4" },
          React.createElement("h3", { className: "text-base font-medium mb-3" }, "Adicionar SKU"),
          React.createElement(
            "div",
            { className: "grid gap-3 md:grid-cols-4" },
            React.createElement("input", {
              className: "border rounded-xl px-3 py-2",
              placeholder: "Código (000.00.00.0000)",
              value: fCodigo,
          ...
          React.createElement(
            ...
            React.createElement(
              "button",
              { className: "px-4 py-2 rounded-xl bg-black text-white", onClick: handleAddSKU },
              "Adicionar SKU"
            ),
            React.createElement(
              "span",
              ...
              "Dica: depois use ",
              React.createElement("b", null, "Exportar JSON"),
              " para salvar a base com os novos itens."
            )
          )
        ),
            ...
            React.createElement(
              ...
              parsed.length === 0
                ? React.createElement(
                    ...
                    React.createElement(
                      "td",
                      { colSpan: 4, className: "p-6 text-center text-gray-500" },
              ...
              parsed.map((r) => {
                ...
                return React.createElement(
                  ...
                  React.createElement(
                    "td",
                    { className: "p-3 mono text-xs md:text-sm whitespace-nowrap" },
                    r.codigo
                  ),
                          ...
                          React.createElement(
                            "button",
                            {
                              ...
                              onClick: () => {
                                ...
                                arr.splice(idx, 1);
                                const copy = { ...mapping };
                                if (arr.length) copy[r.codigo] = arr;
                                else delete copy[r.codigo];
                                setMapping(copy);
                              },
                              className:
                                "absolute -top-2 -right-2 hidden group-hover:block bg-red-600 text-white rounded-full w-6 h-6 text-xs",
                            },
                            "✕"
                          )
                        )
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      "label",
                      { className: "px-3 py-2 rounded-xl border cursor-pointer inline-block" },
                      "Adicionar foto",
                      React.createElement("input", {
                        type: "file",
                        accept: "image/*",
                        multiple: true,
                        className: "hidden",
                        onChange: (e) => handleAttachPhoto(e.target.files, r.codigo),
                      })
                    ),
                    React.createElement(
                      "button",
                      {
                        ...
                        onClick: () => handleDeleteSKU(r.codigo),
                        title: "Excluir SKU",
                      },
                      "🗑️ Excluir"
                    )
        ...
        React.createElement(
          ...
          React.createElement(
            "button",
            { className: "px-4 py-2 rounded-xl bg-black text-white", onClick: exportJSON },
          ...
          React.createElement(
            "button",
            { className: "px-4 py-2 rounded-xl bg-gray-800 text-white", onClick: exportZIP },
            "Baixar ZIP com fotos + manifest"
          )
        ),
        ...
        React.createElement(
          ...
          "Tudo roda no seu navegador (client-side). Nenhum arquivo é enviado a servidores."
        )
      )
    );
  }
